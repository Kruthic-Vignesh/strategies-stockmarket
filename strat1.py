...
def test_different_pfsize(limit = 10):
    ...
    for i in range(2, limit, 2):
        ...
        returned = rebalance_strat(return_df, i, i//2)
        
        returned = returned[1:]
        
        cagr_strat1 = CAGR(returned)
        
        if cagr_strat1 > cagr_max:
            cagr_max = cagr_strat1
            count_max = i
        
    return cagr_max, count_max

dif_pf_size_cagr, dif_pf_size_count = test_different_pfsize(limit = 20)
   ...
   # print("money start ", money)
    for i in range(1, len(df)): # no of months
        if len(portfolio) > 0:
          ...
          #  print(round(monthly_ret[-1], 3), end = " ")
           # print(round(money))
            #for stock in portfolio:
             #   print(stock[:4], end = " ")
              #  print(round(df[stock].iloc[i], 3), end = " ")
            #print()
